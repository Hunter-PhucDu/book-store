...
import { FiFilter, FiGrid, FiList, FiBookOpen } from "react-icons/fi";
...
export default function CategoryPage({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // Chuyển đổi tên danh mục từ URL (lowercase-with-dashes) sang định dạng thực tế
      const decodedCategory = decodeURIComponent(params.category);

      // Tìm tên danh mục thực tế từ sách (để hiển thị đúng cách viết hoa)
      const allCategories = books.map((book) => book.category);
      const matchedCategory = allCategories.find(
        (category) => category.toLowerCase() === decodedCategory,
      );

      setCategoryName(matchedCategory || decodedCategory);

      // Lọc sách theo danh mục
      const filteredBooks = books.filter(
        (book) => book.category.toLowerCase() === decodedCategory,
      );
